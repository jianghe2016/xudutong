...
public class MNCalendarVertical extends LinearLayout {
    ...
    private void initCalendarDatas() {
        ...
        dataMap = new HashMap<>();
        //计算日期
        mnCalendar_countMonth = mnCalendarVerticalConfig.getMnCalendar_countMonth();
...
//        }
    ...
    private void initAdapter() {
        if (mnCalendarVerticalAdapter == null) {
            ...
            RecyclerView.LayoutManager layoutManager = recyclerViewCalendar.getLayoutManager();
            Calendar cal = Calendar.getInstance();
            int year = cal.get(Calendar.YEAR);
            int month = cal.get(Calendar.MONTH) + 1;
            layoutManager.scrollToPosition((year - 2015) * 12 + month-1);
        } else {
