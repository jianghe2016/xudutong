...
//    protected int mSelectedBeginYear = -1;
    ...
    private List<SimpleMonthAdapter.CalendarDay> mCanClickDays = new ArrayList<>();
    ...
    public SimpleMonthView(Context context, TypedArray typedArray) {
        ...
        mRowHeight = ((typedArray.getDimensionPixelSize(R.styleable.DayPickerView_calendarHeight, resources.getDimensionPixelOffset(R.dimen.calendar_height)) - MONTH_HEADER_SIZE) / 2);
    ...
    protected void drawMonthNums(Canvas canvas) {
        ...
        while (day <= mNumCells) {
            ...
            for (int i = 0; i < mSelectedDays.size(); i++) {
                if (day == mSelectedDays.get(i).day) {
                    isSelector = true;
                    break;
                }
            }
            boolean isCanClicked = false;
            for (int i = 0; i < mCanClickDays.size(); i++) {
                if (mYear == mCanClickDays.get(i).year && mMonth == mCanClickDays.get(i).month && day == mCanClickDays.get(i).day) {
                    isCanClicked = true;
                    break;
                }
            }
...
//            }
    ...
    public void setSelectDayList(List<SimpleMonthAdapter.CalendarDay> list) {
        this.mSelectedDays = list;
    }

    public void setCanClickDays(List<SimpleMonthAdapter.CalendarDay> list) {
        this.mCanClickDays = list;
    }
